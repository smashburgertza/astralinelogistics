...
export function CreateBillToAgentDialog({
  open,
  onOpenChange,
}: CreateBillToAgentDialogProps) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      due_date: "",
      notes: "",
      line_items: [{ product_service_id: "", description: "", quantity: 1, unit_price: 0 }],
    },
  });
  ...
  const addLineItem = () => {
    ...
    form.setValue("line_items", [
      ...currentItems,
      { product_service_id: "", description: "", quantity: 1, unit_price: 0 },
    ]);
  };
  ...
  const createInvoiceMutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      // Create invoice with to_agent direction (agent owes Astraline)
    ...
    onSuccess: () => {
      toast.success("Invoice created - agent has been billed");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Bill Agent</DialogTitle>
          <DialogDescription>
            Create an invoice to bill an agent for services provided by Astraline.
          </DialogDescription>
        </DialogHeader>
                          ...
                          <SelectContent>
                            {productsServices?.map((product) => (
                              <SelectItem key={product.id} value={product.id}>
                                {product.name} - {product.currency} {product.unit_price}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
